...
4. PR6b eoPortal EO-constellation extraction: DONE 2026-07-09 (interactive first pass + scheduled continuation). ICEYE page verified (lives at `iceye-constellation`; bare `iceye` is a soft 404 that returns 200 with an empty shell). 13 profiles backfilled verbatim: iceye, capella, umbra, superdove, skysat, pleiades, pleiades-neo, sentinel-1, sentinel-2, satellogic, synspective, pelican, hawkeye-360. prompts/maintain-registry.md v0.3.0 adds the continuing extraction task (up to 5 profiles per weekly run, soft-404/range/design-aim traps documented) for the ~30 EO profiles remaining; some (ghgsat, blacksky, tanager) had no findable eoPortal page on 2026-07-09.
5. PR6c connectivity via ITU/FCC: STARTED 2026-07-09 (commit 3db2d9f): Starlink Ku/Ka/V/E per FCC DA-26-36 and Kuiper Ka/V per FCC DA-26-553, both official record SNR 5, orders fetched from docs.fcc.gov and read directly. Remaining: OneWeb (no filing fetched; SpaceNews rate-limited and fcc.report Cloudflare-blocked on this network 2026-07-09), capacity/user_terminals/service_type fields for all three. Also done 2026-07-09: first-party imaging-modes seeding (ICEYE/Capella/Umbra tables from operator product pages, SkySat swath conflict resolved first-party, SuperDove 8 named bands from the PSScene spec PDF) and eoPortal batch 2 (16 more profiles, 29 of 44 EO constellations now carry specs; axelspace-grus has no eoPortal page; fleet parents documented as skipped).
...
STATUS: MERGED to main 2026-07-09 as PR #104 (squash 0d7ba0a) after Florian's approval, seven review rounds deep (v3 through v3.5d). Key late decisions now codified in CLAUDE.md: frames belong to content blocks, sections unfenced (heading + top hairline); numbers at display size, stated phrases at body scale. The ON ORBIT module (hero count + launch-year step chart from the element set's OBJECT_ID years) is its own section, not a key-specs cell. Remaining queue lives in section 9 (Florian: logo review, accent sign-off; joint: mcc_read pass; Claude: first-party imaging-modes + sats_planned seeding pass, PR6c ITU/FCC, deploy).
...
v3.2 refinements (Florian's second review, same PR): sourcing mark flush right; the orbit tab overlays the REAL /orbits/ globe (mini3d, worker-propagated, click-to-popup) over the SVG fallback, frame stretched to the facts column, dots scaled 3.5x (attenuated point size scales with canvas height, sub-pixel in small embeds); specs & sensors = mode cards + fact-cell grid; history = events (dimmed date tags) + TIMELINE (spine, year markers, type filters); sources = provenance cards with column-aligned marks; key-specs band gains a full-width "on orbit by launch year" cumulative step chart computed client-side from the committed element set's OBJECT_ID launch years (honest framing: today's catalog by launch year, decayed sats absent). ICEYE carries the ScanSAR (under development) mode row. PLANNED next: first-party imaging-modes pass (operator product pages state modes eoPortal omits, e.g. ICEYE Spot Fine/Dwell); the rule is already in prompts/maintain-registry.md, an interactive first batch would seed it.
