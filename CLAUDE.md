...
- Corroboration raises the score, once per rule: a second distinct source, a fourth, and pickup by a mainstream (non-trade) outlet beyond the lead. Wire rewrites of one story are one source. A corroboration crawl that ran and found nothing costs one level (indirect leads only: a direct source proves its own statement); a crawl that never ran costs nothing, and the gate rejects skipped crawls the budget covered.
