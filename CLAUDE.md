...
- `noise`: worth logging, not worth a push. Examples: a scheduled launch succeeding on schedule; a routine product update; a minor partnership without stated money, capacity, or regulatory effect. Routine product updates, minor partnerships, and scheduled successes default here, however long the press release. Routine megaconstellation batch launches (Starlink, SpaceSail/G60, Guowang, Kuiper and peers) publish here and are never discarded as cadence, US and Chinese alike (Florian, 2026-07-12).
- When torn between two tiers, take the lower one; the feed's credibility spends on restraint. Impact for commentary caps at `major` (2026-07-13; seismic is events-only).
...
1. The best candidate from the item's own sources, tried press-first (trade > mainstream > aggregator > informal > first-party > official record): trade-press artwork beats press-release, investor-relations, and filing pages (Florian, 2026-07-08). Candidates are each page's og:image / twitter:image PLUS its in-article figures (Florian, 2026-07-13: a publisher's generic og:image header must not beat the real photograph in the article body). When an item has more than one gate-passing candidate, a sealed bounded judge step in the sweep workflow (Haiku, Read-only plus one Write, fixed rubric: real photograph of the subject > any real photograph > concept art or render > abstract or stock graphic > chart, diagram, or logo) ranks them, one line of reasoning logged per item; if the judge fails or writes nothing, the og-image-first order applies unchanged. The judge only ever ranks candidates the deterministic gates already passed; it cannot introduce an image. Social platform pages never contribute their own og:image (it is a profile picture); a Bluesky post resolves to the article it embeds via the public API and that page joins the candidates. PDFs are skipped; tiny images and ad-shaped banners are rejected; a logo-shaped image is used only when no source yields a photo. The winner is re-hosted under `public/img/items/`, credited to and linked to the page it actually came from.
...
- Positioning claims are first-party only (2026-09-09): a claim's source must sit on the entity's own registry-recorded website domain; the validator rejects any other. Maintenance runs fill empty claims blocks from the entity's own pages, a few profiles per run.
- Organizations self-populate from the feed (Florian, 2026-09-09): organization profiles carry `headquarters`, `parent_org`, `funding_latest`, `funding_total`, `valuation_latest`, and `employees` alongside the original fields, all crossfeed-eligible; a funding, M&A, IPO, headquarters, or status item on a profiled organization must attest the matching fact. Agreement is never a dispute: a claim stating the stored value confirms it (a stronger source refreshes the citation). Consumed crossfeed candidates are recorded with their outcome in `src/data/registry-crossfeed-log.json` (machine-written, rendered on /system), and companies the feed keeps naming without a profile surface in `src/data/registry_suggestions.json` for Florian's review; profiles are still only created through reviewed changes.
